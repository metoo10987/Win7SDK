...
{
	string str;
	string src = "赵海彬，1你好啊，这是什么地方啊0123456789ABCDEFJHIGH中国";
	

	string desc;
	const char* p = NULL;
	string temp;
	string_separater<int> str_separater(src,3);
	for(;str_separater.get_next_string(temp); )
		desc += temp;
	desc += temp;

	desc = "";
	string src_1 = "123455123451234512345123";
	string_separater<char> str_separater_1(src_1,'5');
	for(; str_separater_1.get_next_string(temp); )
		desc += temp;
	desc += temp;
	return 0;
}