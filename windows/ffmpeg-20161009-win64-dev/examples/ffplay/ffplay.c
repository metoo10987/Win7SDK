    ...
    do {
        ...
        if (got_frame) {
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
                ...
                af->duration = av_q2d(/*(AVRational){frame->nb_samples, frame->sample_rate}*/avr);
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (ret >= 0) {
            ...
            duration = (frame_rate.num && frame_rate.den ? av_q2d(/*(AVRational){frame_rate.den, frame_rate.num}*/avr) : 0);
...
static void seek_chapter(VideoState *is, int incr)
{
	...
	AVRational avr; avr.num = 1; avr.den = AV_TIME_BASE;
    ...
    for (i = 0; i < is->ic->nb_chapters; i++) {
        ...
        if (av_compare_ts(pos, /*AV_TIME_BASE_Q*/avr, ch->start, ch->time_base) < 0) {
            i--;
            break;
        }
    }
